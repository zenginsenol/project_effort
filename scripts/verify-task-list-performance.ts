 ...
 * Usage: npx tsx scripts/verify-task-list-performance.ts [projectId]
 */

import { db } from '../packages/db/src/index';
import { tasks } from '../packages/db/src/schema';
...
async function verifyTaskListPerformance(projectId: string) {
  try {
    ...
    if (taskCount < 500) {
      ...
      console.error(`Run: npx tsx scripts/seed-500-tasks.ts`);
      process.exit(1);
    }
    ...
    const initialPage = await db.query.tasks.findMany({
      where: eq(tasks.projectId, projectId),
...
if (!projectId) {
  ...
  console.error('Usage: npx tsx scripts/verify-task-list-performance.ts [projectId]');
  console.error('  Or set TEST_PROJECT_ID environment variable');
  console.error('\nRun: npx tsx scripts/seed-500-tasks.ts to create a test project');
  process.exit(1);
}
